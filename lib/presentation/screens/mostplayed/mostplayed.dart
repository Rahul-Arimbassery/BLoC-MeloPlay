...
import 'package:google_fonts/google_fonts.dart';
import 'package:musicuitest/bloc/mostplayed/bloc/mostplaed_bloc_bloc.dart';
import 'package:on_audio_query/on_audio_query.dart';
import 'package:shared_preferences/shared_preferences.dart';
...
class MostPlayedPage extends StatelessWidget {
  ...
  MostPlayedPage({Key? key}) : super(key: key) {
    initialize();
  }
  ...
  Future<void> _initPreferences() async {
    ...
    songIds =
        _prefsMostPlayed.getStringList('songIds')?.map(int.parse).toList() ??
            [];
    playCountMap = {
      for (var id in songIds) id: _prefsMostPlayed.getInt('$id') ?? 0
    };
  }
  ...
  Widget build(BuildContext context) {
    return BlocProvider(
      create: (context) => MostplaedBlocBloc(context),  //Implementing Bloc Logic Here..
      child: Scaffold(
        appBar: AppBar(
          shadowColor: const Color.fromARGB(255, 27, 164, 179),
          elevation: 10,
                          ...
                          child: InkWell(
                            ...
                            child: ListTile(
                              ...
                              trailing: Row(
                                mainAxisSize: MainAxisSize.min,
                                children: [
                                  BlocBuilder<MostplaedBlocBloc, MostplaedBlocState>(
                                    builder: (context, state) {
                                      return IconButton(
                                        onPressed: () async {
                                          ...
                                          //   ),
                                          // );
                                          BlocProvider.of<MostplaedBlocBloc>(context).add(NavigateEvent(currentindex));
                                        },
                                        icon: const Icon(
                                          Icons.play_arrow,
                                          size: 30,
